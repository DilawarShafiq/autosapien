        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-10 mb-12 sm:mb-16">
          ...
          <div className="col-span-2">
            <Link to="/" className="inline-flex items-center mb-5">
              <img src="/logo.png?v=2" alt="Autosapien" className="h-10 w-auto" />
            </Link>

            <p className="text-ink-400 font-body text-sm mb-6 max-w-xs leading-relaxed">
            ...
            <div className="flex gap-2">
              {['linkedin', 'twitter', 'github', 'youtube'].map((social) => (
                <a
                  key={social}
                  href={`#${social}`}
                  className="w-8 h-8 rounded-full border border-surface-300 flex items-center justify-center hover:border-ink-900 hover:bg-ink-900 transition-all group"
                >
                  {social === 'linkedin' && (
                    <svg className="w-3 h-3 text-ink-400 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
                  )}
                  {social === 'twitter' && (
                    <svg className="w-3 h-3 text-ink-400 group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
                  )}
                  {social === 'github' && (
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="label-mono text-ink-900 mb-4 capitalize">{title}</h4>
