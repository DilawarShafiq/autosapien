...
export function Footer() {
  ...
  return (
    <footer className="relative section-dark gradient-border-top overflow-hidden">
      {/* Noise overlay */}
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-6 sm:pb-10">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-10 mb-14 sm:mb-20">
          {/* Brand */}
            ...
            <div className="flex gap-2">
              {['linkedin', 'twitter', 'github', 'youtube'].map((social) => (
                <a
                  key={social}
                  href={`#${social}`}
                  className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:border-sky-500/50 hover:bg-sky-500/10 transition-all group"
                  aria-label={social}
                >
                  {social === 'linkedin' && (
                    <svg className="w-3.5 h-3.5 text-white/35 group-hover:text-sky-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="label-mono-white text-white/60 mb-5 capitalize">{title}</h4>
                ...
                {links.map((link) => (
                  <li key={link.name}>
                    {'isRoute' in link && link.isRoute ? (
                      <Link
                        to={link.href}
                        className="text-sm text-white/35 font-body hover:text-white transition-colors"
                      >
                        {link.name}
                      </Link>
                    ) : (
        ...
        <div className="pt-6 border-t border-white/6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-5">
              ...
              <button
                onClick={scrollToTop}
                className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center hover:border-sky-500/40 hover:bg-sky-500/10 transition-all group"
                aria-label="Scroll to top"
              >
                <ArrowUp className="w-3.5 h-3.5 text-white/30 group-hover:text-sky-400 transition-colors" />
              </button>
            </div>
          </div>
        </div>
