...
export const agenticHealthcareModules: AcademyModule[] = [
    ...
    lessons: [
      ...
      },
      {
        ...
        keyInsights: [
          ...
          'CoverMyMeds API: largest pharmacy PA clearinghouse; REST API for drug PAs; covers 95% of commercially insured lives; significant pharmacy PA automation through this single integration',
        ],
...

  {
    ...
    subtitle: 'xEHR.io, the Autosapien agentic platform & the RCM Automation Platform',
  ...
  {
    number: 9,
    slug: 'capstone-ship-digital-fte',
    title: 'Capstone: Ship a Digital FTE',
    subtitle: 'From Zero to a Running Eligibility Agent',
    description: 'Turn the book into a deployed agent: scope a Digital FTE, build an eligibility agent end to end with Claude and real payer transactions, and take it to production safely.',
    duration: '5 hours',
    level: 'Advanced',
    accentColor: 'sky',
    bgColor: '#f0f7ff',
    prerequisites: ['eligibility-prior-auth-agents', 'hipaa-compliant-ai-deployment'],
    outcomes: [
      'Scope a Digital FTE with a clear role, tools, and escalation rules',
      'Build an eligibility agent end to end with Claude tool use and real EDI/FHIR transactions',
      'Take a healthcare agent to production with a HIPAA and go-live checklist',
    ],
    lessons: [
      {
        id: 'H9.1',
        ...
        duration: '40 min',
        overview: 'Before writing code, scope the agent the way you would hire an employee: one clear role, the tools it may use, the metrics it is accountable to, and exactly when it must escalate to a human. A well-scoped FTE is the difference between a demo and a deployable worker.',
        keyInsights: [
          'Pick ONE high-volume, structured role first: eligibility verification is the best starting point (roughly 70-80% automatable, clean 270/271 EDI, high daily volume)',
          'Define the FTE like a job description: role, the exact tools and APIs it may call, its inputs and outputs, performance SLAs (turnaround, accuracy), and escalation triggers',
          ...
          'Choose the orchestration model and tools up front: a Claude model as the reasoning core plus a small explicit tool registry (eligibility API, EHR read, logging) beats an open-ended agent',
          'Write the scope as a structured spec (a SKILL.md-style file): role, permissions, escalation, output format, SLAs; it becomes both documentation and the backbone of the agent system prompt',
          'Define success metrics before building: clean-result rate, escalation rate, and turnaround versus the human baseline, so you can prove the FTE works in shadow mode',
        ],
      ...
      },
      {
        ...
        duration: '60 min',
        overview: 'The hands-on build: a working eligibility Digital FTE. You will wire a Claude model to a small tool set, run a real 270 eligibility request and parse the 271 response, and return a structured coverage answer with a human-escalation path, the full loop end to end.',
        keyInsights: [
          'Architecture: a Claude model as the reasoning core plus tool use (function calling) to call an eligibility API, read patient and insurance context, and write an audit log; the model orchestrates, the tools act',
          'The core transaction: submit a HIPAA 270 eligibility request to a clearinghouse (for example through the Availity API) and parse the 271 response into a structured coverage result (active or inactive, copay, deductible, plan)',
          'Tool design: keep tools small and deterministic (submit_eligibility, get_patient, log_result); the model decides WHEN to call them and the tools encapsulate the payer-specific details',
          'Grounding and guardrails: never let the model invent coverage facts; it reports only what the 271 returns, and low-confidence or unparseable responses escalate to a human',
          'Audit-trail-first: every tool call and decision is logged with inputs, outputs, and timestamps for HIPAA accountability and later evaluation',
          'Build with the current Anthropic tool-use API: define tools as JSON schemas, let the model emit tool calls, execute them, and feed results back until it returns the final structured answer',
        ],
        lab: 'Build an eligibility agent: define three tools (submit_eligibility returning a parsed 271, get_patient, log_result) as JSON schemas, wire them to a Claude model via tool use, run an end-to-end verification on a test patient, and add an escalation path for low-confidence responses. Use synthetic or test data only, never real PHI.',
      ...
      },
      {
        ...
        duration: '45 min',
        overview: 'A working agent on your laptop is not a production Digital FTE. This lesson is the go-live checklist: the HIPAA, safety, monitoring, and rollout steps that turn a prototype into a worker you can trust with real patients and real money.',
        keyInsights: [
          'HIPAA technical baseline: encryption in transit and at rest, least-privilege access, a signed BAA with every vendor in the data path, and PHI handling controls (detection and masking where appropriate)',
          'Start in shadow mode: run the agent in parallel with the human workflow for about two weeks, comparing its output to human decisions, and graduate to autonomous only when accuracy clears your threshold (for example above 95%)',
          'Escalation and rollback: explicit dollar and confidence thresholds route cases to humans, and a kill switch plus rollback plan let you revert instantly if behavior degrades',
          'Monitoring: a per-agent dashboard of volume, clean-result rate, escalation rate, turnaround, and cost versus baseline, with alerts on drift rather than only on failures',
          'Incident response: define what counts as an incident (wrong coverage, PHI exposure), who is paged, and the steps to contain and remediate',
          'Iterate from real data: the logs the agent produces are the richest source of edge cases; feed misses back into the tools, prompts, and escalation rules each cycle',
        ],
        lab: 'Write a go-live checklist for your eligibility agent: list the HIPAA controls, the shadow-mode plan and graduation threshold, the escalation and kill-switch design, the monitoring dashboard metrics, and the incident-response runbook. Then dry-run one simulated incident through it.',
